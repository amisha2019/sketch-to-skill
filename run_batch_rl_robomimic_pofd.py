...
root_dir = "/fs/nexus-projects/Sketch_VLM_RL/amishab/IBRL_robomimic_sketch_to_skill1/"
...
for seed in seeds:
    ...
    python_command = (
        f"python train_rl_pofd.py "
        f"--config_path /fs/nexus-scratch/amishab/Teacher_student_RLsketch/release/cfgs/robomimic_rl/can_ibrl.yaml "
        f"--seed {seed} "
        f"--save_dir {root_dir}/can_seed{seed} "
        f"--use_wb 0 "
        f"--is_demo False"  # only for sketch
    )
