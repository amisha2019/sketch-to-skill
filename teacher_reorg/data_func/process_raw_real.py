...
def load_raw_data(file_path, num_samples=None):
    ...
    with h5py.File(file_path, 'r') as f:
        ...
        for i in tqdm(index, desc="Loading data"):
            ...
            sketches1.append(obs_group['corner_blank_sketch'][:])
            sketches2.append(obs_group['corner2_blank_sketch'][:])
            prop = obs_group['prop']
            props.append(prop[:])
            trajectories.append(prop[:, :3])
            rgbs1.append(obs_group['corner_image'][0])
            rgbs2.append(obs_group['corner2_image'][0])
            rgb_sketch1.append(obs_group['corner_rgb_sketch'][:])
            rgb_sketch2.append(obs_group['corner2_rgb_sketch'][:])
    ...
    return sketches1, sketches2, rgbs1, rgbs2, rgb_sketch1, rgb_sketch2, trajectories, props, states
...
def load_samples_and_save(file_path, save_path, file_name, img_size=[320], num_control_points=[10, 20, 50], num_samples=None):

    save_path = f"{save_path}/{file_name}"
    os.makedirs(save_path, exist_ok=True)

    stime = time.time()
    file_path = f"{file_path}/{file_name}.hdf5"
    sketches1, sketches2, rgbs1, rgbs2, rgb_sketch1, rgb_sketch2, trajectories, props, states = load_raw_data(file_path, num_samples=num_samples) 
...
if __name__ == "__main__":
    file_path = "/fs/nexus-projects/Sketch_VLM_RL/amishab/demo_dataset_real"
