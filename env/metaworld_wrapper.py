...
class MetaWorldEnv(gym.Env):
    ...
    def set_obj_pose_sktchRL(self, obj_pose, goal_pose):
        print(f"******** Setting Object Init Position: {obj_pose}, Goal Position: {goal_pose}")
        self.env.init_config['obj_init_pos'] = obj_pose - np.array([0, -0.18587194, 0.09])
        # # self.env.obj_init_pos = obj_pose
        # self.env.goal = goal_pose
        # self.env._target_pos = goal_pose
        # # |-- offset [not-needed] --|
        ...
        # print(f"******** First Resetting Obs: {obs[4:7]}, Goal: {obs[-3:]}")

        # for i in range(1):
        #     # calculate the offset and apply init again
        #     obs_offset = obs[4:7] - obj_pose
        #     goal_offset = obs[-3:] - goal_pose
        #     new_obj_pose = obj_pose - obs_offset
        #     new_goal_pose = goal_pose - goal_offset
            
        #     self.env.init_config['obj_init_pos'] = new_obj_pose
        #     # self.env.obj_init_pos = new_obj_pose
        #     self.env.goal = new_goal_pose
        #     self.env._target_pos = new_goal_pose
        #     self.env.reset_model()
        #     obs = self.env._get_obs()
        #     print(f"******** After Offset Obs: {obs[4:7]}, Goal: {obs[-3:]}")
        # breakpoint()
...
class PixelMetaWorld:
    ...
    def set_obj_pose_sktchRL(self, obj_pose, goal_pose):
        self.env.env.set_obj_pose_sktchRL(obj_pose, goal_pose)
    ...
    def _extract_images(self, obs):
        ...
        # Change state to only 7 dimension
        # -------------------------------------------------------
        # ee_pose = obs["state"][:4]  # Move tensor to CPU
        # goal_pose = obs["state"][-3:]  # Move tensor to CPU
        # obs["state"] = np.concatenate([ee_pose, goal_pose], axis=0)
        # ---------------------------------------------------------------------        
