...
class PixelRobosuite:
    ...
    def _extract_images(self, obs):
        ...
        # rl_obs["obj"] = torch.from_numpy(np.concatenate([obs["SquareNut_pos"], obs["SquareNut_quat"]]).astype(np.float32))
        rl_obs["obj"] = torch.from_numpy(obs["object-state"][:7].astype(np.float32))

        # change everything to cuda
        for key in rl_obs:
            rl_obs[key] = rl_obs[key].to(self.device)
