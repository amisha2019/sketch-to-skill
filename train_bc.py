...
def run(cfg: MainConfig, policy):
    ...
    if policy is None:
        if cfg.dataset.use_state:
            ...
            print(f"Dataset observation shape: {dataset.obs_shape}")
            print(f"Dataset property shape: {dataset.prop_shape}")
            print(f"Dataset action dimension: {dataset.action_dim}")
...
if __name__ == "__main__":
    ...
    cfg.save_dir = f"/fs/nexus-projects/Sketch_VLM_RL/amishab/BC_robomimic_sketch/can_seed{cfg.seed}"
