...
class Discriminator(nn.Module):
    def __init__(self, input_size):
        # input_size = 13, x, y, z, dx, dy, dz, obj_x, obj_y, obj_z, obj_quat_x, obj_quat_y, obj_quat_z, obj_quat_w
        super().__init__()
    ...
    def process_demo_traj(self, demo_traj, bs):
        ...
        obj = demo_traj["obj"]              # shape: (data_size, 7)
        ...
        expert_input = torch.cat([xyz, d_xyz, obj], dim=-1)  # shape: (bs, 13)
        expert_input = expert_input.reshape(-1, 13)  # shape: (bs, 13)

        return expert_input

    def process_batch(self, batch):
        obs: dict[str, torch.Tensor] = batch.obs   #[256, 27]
        next_obs: dict[str, torch.Tensor] = batch.next_obs  #[256, 27]

        xyz = obs["prop"][:, :3]
        next_xyz = next_obs["prop"][:, :3]
        d_xyz = next_xyz - xyz
        d_xyz = d_xyz / (torch.norm(d_xyz, dim=-1, keepdim=True) + 1e-8)
        obj = obs["obj"]
        
        policy_input = torch.cat([xyz, d_xyz, obj], dim=-1)  # shape: (bs, 13)
        policy_input = policy_input.reshape(-1, 13)  # shape: (bs, 13)
...
class MainConfig(common_utils.RunConfig):
    ...
    add_bc_loss: int = 0
    # discriminator
    load_demo_traj: str = "/fs/nexus-projects/Sketch_VLM_RL/amishab/datasets_robomimic/can/ph/96s/processed_96_demo20_servoing_obj.hdf5"
...
class Workspace:
    def __init__(self, cfg: MainConfig, from_main=True):
        ...
        self.load_demo_traj()
        self.discriminator = Discriminator(input_size=13)
    ...
    def load_demo_traj(self):
        ...
        with h5py.File(self.cfg.load_demo_traj, "r") as f:
            ...
            for key in data_group.keys():
                obs_group = data_group[key]['obs']
                traj = obs_group['robot0_eef_pos'][:]
                obj = obs_group["object"][:,:7]
                xyz.append(traj[:-1, :])
                next_xyz.append(traj[1:, :])
                obj_list.append(obj[:-1, :])
        xyz = np.concatenate(xyz, axis=0)
        next_xyz = np.concatenate(next_xyz, axis=0)
        obj_list = np.concatenate(obj_list, axis=0)
        ...
        self.demo_traj = {
            ...
            "obj": torch.tensor(obj_list).float().to("cuda:0")
        }

    def _setup_env(self):
        # self.rl_cameras: list[str] = list(set(self.cfg.rl_cameras + self.cfg.bc_cameras))
        self.rl_cameras: list[str] = list(set(self.cfg.rl_cameras + ["robot0_eye_in_hand"]))
...
def main():
    ...
    cfg.save_dir = f"/fs/nexus-projects/Sketch_VLM_RL/amishab/IBRL_robomimic_sketch_to_skill1/can_seed{cfg.seed}"
