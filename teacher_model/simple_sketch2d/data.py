...
def trajectory_to_sketch(trajectory, traj_min, traj_max, image_size=64, line_width=3, padding=4):
...
def generate_dataset(num_samples=1000, num_control_points=40, trajectory_types=['sine', 'spiral', 'cubic', 'logarithmic_spiral'], max_displacement=2.0, min_size=2.0, max_size=8.0, fit_bspline=False):
    sketches = []
    trajectories = []
    if fit_bspline:
        control_points = []
    ...
    for i in range(num_samples):
        ...
        # traj_type = trajectory_types[i % 2 + 2]
        ...
        if traj_type == 'sine':
            ...
            t_range = (0, np.random.uniform(2*np.pi, 4*np.pi))
    ...
    for traj in trajectories:
        sketch = trajectory_to_sketch(traj, traj_min, traj_max)
...
def get_dataloader(batch_size=32, num_samples=1000, num_control_points=40, max_displacement=2.0):
    sketches, trajectories, params, fitted_trajectories = generate_dataset(num_samples=num_samples, num_control_points=num_control_points, max_displacement=max_displacement, min_size=2.0, max_size=8.0, fit_bspline=True)
    # converting sketches to control points (params) to tensor
    ...
    # print(f"Expected n_dim_total: {model.n_dim_total}")
...
if __name__ == "__main__":
    ...
    sketches, trajectories, control_points, fitted_trajectories = generate_dataset(num_samples=8, num_control_points=10, max_displacement=2.0, min_size=2.0, max_size=8.0, fit_bspline=True)
