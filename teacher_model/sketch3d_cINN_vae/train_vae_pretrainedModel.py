...
def train_vae_pretrained(model, optimizer, train_loader, val_loader, num_epochs, root_dir, logging_file, args):
...
def visualize_evaluation(model, test_loader, name, root_dir):
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_loader):
            ...
            visualize_sketches(selected_sketches, selected_recons, f"reconstructed_sketches_{i}", root_dir)
...
def visualize_sketches(sketches, reconstructed_sketches, img_name, root_dir):
    ...
    for idx, (sketch, recons) in enumerate(zip(sketches, reconstructed_sketches)):
        ...
        row = idx // cols * 2
        col = idx % cols
        axs[row, col].imshow(sketch)
        axs[row, col].set_title("Original Sketch")
        axs[row, col].axis('off')
        axs[row+1, col].imshow(recons)
...
if __name__ == "__main__":    
    ...
    # parser.add_argument('--gamma', type=float, default=0.5, help='Gamma value for scheduler')
    parser.add_argument('--bs', type=int, default=256, help='Batch size for training')
    parser.add_argument('--M_N', type=float, default=0.00005, help='kld weight for loss function')
    ...
    parser.add_argument('--layersOfPreTrainedModel', type=int, default=4, help='number of layers of pretrained model')
    ...
    if train_vae_flag:
        ...
        unique_name = f"vae_with_{args.typeOfPretrainedModel}PlusOneMlp_{args.layersOfPreTrainedModel}Layer{'Frozen' if args.ifFreezePretrainedModel else 'Trainable'}_{unique_token}_ep{args.num_epochs}_{args.scheduler}_lr{args.lr}_bs{args.bs}_kld{args.M_N}_aug"
    else:
        unique_name = "vae_2024-09-19_13-56-26_lr1e-3_gamma0.5_bs512_aug"

    if args.root_dir is None:
        root_dir = f"/fs/nexus-projects/Sketch_VLM_RL/teacher_model/{os.environ.get('USER')}/VAE_pretrained_singleTest/{unique_name}"
    else:
    ...
    img_size = 64
    model = VAE(img_size=img_size, in_channels=3, latent_dim=256, ifPretrained=True, preTrainedModel_type=args.typeOfPretrainedModel, preTrainedModel_layers=args.layersOfPreTrainedModel, freeze=args.ifFreezePretrainedModel).cuda()
    ...
    if train_vae_flag:
        ...
        train_vae_pretrained(model, optimizer, sketch_train_loader, sketch_val_loader, args.num_epochs, root_dir, logging_file, args)

        # Save the trained model
        torch.save(model.state_dict(), f'{root_dir}/models/vae_model_{img_size}.pth')
    else:
        # Load the best model
        model.load_state_dict(torch.load(f'{root_dir}/models/vae_model_best.pth'))
