...
####Sketch##################
    ...
    # "ButtonPress": "/fs/nexus-projects/Sketch_VLM_RL/teacher_model/peihong/raw_demo_servoring_data/ButtonPress/ButtonPress_teachermodel_gen_3.hdf5",
    ...
    # "Reach": "/fs/nexus-projects/Sketch_VLM_RL/teacher_model/peihong/raw_demo_servoring_data/Reach/Reach_teachermodel_gen_3.hdf5",
    ...
    # "ReachWall": "/fs/nexus-projects/Sketch_VLM_RL/teacher_model/peihong/raw_demo_servoring_data/ReachWall/ReachWall_teachermodel_gen_3.hdf5",
    ...
    def __init__(self, cfg: DatasetConfig):
        ...
        for episode_id in range(num_episode):
            ...
            if self.cfg.use_state:
                all_states = []
                ee_pose = episode["obs"]["state"][:, :4]
                goal_pose = episode["obs"]["state"][:, -3:] 

                new_array = np.concatenate([ee_pose, goal_pose], axis=1)  # Concatenate along columns
                print("Concatenation successful. New array shape:", new_array.shape)

                all_states.append(new_array)
        ...
        # self.env.obj_rand_init(rand_init=False)
...
def run(cfg: MainConfig, policy):
    ...
    obs_dim = dataset.obs_shape[0]
    print("obs dim", obs_dim)
    print("Observation Dimension:", dataset.obs_shape)
    print("Action Dimension:", dataset.action_dim)
    print("Policy Config:", cfg.policy)

    policy = StateBcPolicy(obs_dim, dataset.action_dim, cfg.policy)
...
def load_model(weight_file, device):
    ...
    # policy = StateBcPolicy(env.observation_shape, env.action_dim, cfg.policy)
    policy = StateBcPolicy(7, env.action_dim, cfg.policy)
...
if __name__ == "__main__":
    ...
    cfg.save_dir =f"/fs/nexus-projects/Sketch_VLM_RL/amishab/BC_sketch_state_gen5/{cfg.dataset.task_name}_epoch{cfg.num_epoch}_seed{cfg.seed}"
