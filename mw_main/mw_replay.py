...
def add_demos_to_replay(
    ...
    for episode_id in range(num_episode):
        ...
        # Change state dimension to 7
        # # -------------------------------------------------------
        # ee_pose = episode["obs/state"][:, :4]  # Move tensor to CPU
        # goal_pose = episode["obs/state"][:, -3:]  # Move tensor to CPU
        # new_state = np.concatenate([ee_pose, goal_pose], axis=1)
        # --------------------------------------------------------
        
        # states: np.ndarray = np.array(new_state)  # type: ignore
